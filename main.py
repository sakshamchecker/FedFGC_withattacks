...
def execute(args, cr, dp, experiment_path, attacks):
    ...
    print("Data Loaded")
    target_model = DiffPool(feat_dim=dataset.num_features, num_classes=dataset.num_classes, max_nodes=5, args=args)
    # print(target_model.model)
    print("Model Loaded")
    test_accuracy=train_a_model(target_model, dataset, target_indices, attack_test_indices, num_epochs=args.epochs, batch_size=args.batch_size, coarsen=cr, dp=dp, dp_params=[1.1, 0.3], max_nodes=5)
