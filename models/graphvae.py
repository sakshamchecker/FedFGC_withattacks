...
from models.graphvae_net import GraphVAENet


class GraphVAE(GNNBase):
    def __init__(self, feat_dim, embedding_dim, num_classes, max_nodes):
        super(GraphVAE, self).__init__()
        self.logger = logging.getLogger('graph_vae')

        # self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = GraphVAENet(feat_dim, embedding_dim, num_classes, max_nodes)
        # self.data_store = DataStore(args, max_nodes)

    def train_model(self, train_loader, num_epoch=10):
        self.model.train()
        self.logger.info("Trainig VAE")
        ...
        for epoch in range(num_epoch):
            ...
            self.logger.info("Model set to device")
